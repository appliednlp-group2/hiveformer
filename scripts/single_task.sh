...
for seed in 0 1 2
do 
    mkdir -p xp/hiveformer/$task/$seed
    nohup sh scripts/train.sh /root/hiveformer_dataset/multi_task_dataset $task $seed 0 instructions_fixed ${cuda[$seed]} > xp/hiveformer/$task/$seed/log.txt &
done
